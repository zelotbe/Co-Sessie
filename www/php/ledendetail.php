...
$stmt = $db->prepare("SELECT * FROM leden WHERE leden_id=".$_GET['lid_id']);
$stmt->execute();
...
<body>
    ...
    <main class="w-50 mx-auto">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item "><a href="leden.php" class="tekstkleur font-weight-bold">Leden</a></li>
                <li class="breadcrumb-item" aria-current="page">Detail van lid</li>
            </ol>
        </nav>
        <?php if (sizeof($lid) > 0) { ?>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <p class="card-text"><span class="font-weight-bold ">Email:</span> <?php echo htmlentities($lid['mail']);?><br><span class="font-weight-bold ">Adres:</span> <?php echo htmlentities($lid['adres']);?><br><span class="font-weight-bold ">Gemeente + postcode:</span> <?php echo htmlentities($lid['gemeente']);?> <?php echo htmlentities($lid['postcode']);?><br> <span class="font-weight-bold ">Geboortedatum + plaats:</span> <?php echo htmlentities($lid['geboortedatum']);?> <?php echo htmlentities($lid['geboorteplaats']);?><br><span class="font-weight-bold ">Nationaliteit:</span> <?php echo htmlentities($lid['nationaliteit']);?><br><span class="font-weight-bold ">Geslacht:</span> <?php echo htmlentities($lid['geslacht']);?><br><span class="font-weight-bold ">GSM nummer:</span> <?php echo htmlentities($lid['gsm']);?></p>
            </div>
            <div class="card-footer">
                <small class="text-muted">Ingeschreven op: <span class=""><?php echo (new Datetime($lid['inschrijving']))->format('d-m-Y'); ?></span></small>
            </div>
        </div>
        <?php } ?>
        ...
        echo '<p>Deze persoon heeft geen info.</p>' . PHP_EOL;
    }
    ?>
